...
class MainRepositoryImpl @Inject constructor(private val apiService : ApiService): MainRepository {
    ...
    ): Response<LoginResponseModel> = apiService.userLogin(mobile,device_id,device_type,device_name,device_token,referal_code)
    ...
    override suspend fun payment_status_check(
        token: String,
        transaction_id: String
    ): Response<RazorpaystatusResponse> = apiService.payment_status_check(token,
        transaction_id
    )
    ...
    override suspend fun updateUserProfile(token : String,
     ...
     profile_image: MultipartBody.Part?): Response<EditProfileResponse> = apiService.updateUserProfile(token ,name, email, mobile_number
    ...
    override suspend fun add_my_wallet(token: String,amount : String,transactionId:String): Response<WalletResponse> = apiService.add_my_wallet(token,amount,transactionId)
    ...
    override suspend fun searchLoaderDetailApi(
        ...
        pickup_lat, pickup_long, dropup_location, dropup_lat, dropup_long, booking_date,booking_time ,available)
...
//        seat: String,
 ...
 override suspend fun reschedule_trip_passenger(
        ...
        booking_date: String,
        booking_time: String
    ): Response<LoaderRescheduleTripResponseModel> = apiService.reschedule_trip_passenger(token, booking_id,booking_date,booking_time )
    ...
    ): Response<OngoingLoaderTripHistoryResponseModel> = apiService.loaderPendingBookingTripHistoryApi(token)
    ...
    override suspend fun razorpayPayment(
        ...
        currency: String,
        booking_relation_id: String
    ): Response<RazorpaystatusResponse> = apiService.razorpayPayment(token, pick_up_location,
    ...
    override suspend fun loaderWalletListApi(token: String,date:String,transaction_type: String): Response<LoaderWalletListResponseModel>
    ...
    ): Response<OngoingPassengerTripHistoryResponseModel> = apiService.passengerPendingBookingTripHistoryApi(token)
...
override suspend fun get_user_check_statusApi(
        token: String
    ): Response<AuthorisedFranchisesStateListResponseModel> = apiService.get_user_check_statusApi(token)
